...
<%
    ...
    class ScheduleEntry {
        String name, faculty, room;
        boolean isLab;
        ScheduleEntry(String n, String f, String r, boolean l) {
            this.name = n; this.faculty = f; this.room = r; this.isLab = l;
        }
    }
%>
...
<head>
    <meta charset="UTF-8">
    <title>Compact Schedule &#124; K.I.T. Kolhapur</title>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        @media print {
            .small-box-frame { transform: scale(1); border: none; box-shadow: none; width: 100%; }
    ...
    <div class="small-box-frame">
        <div class="header">
            <div class="actions no-print">
                ...
                <a href="ExportStudentPDFServlet" class="btn-action"><i class="bi bi-file-pdf"></i> PDF</a>
            </div>
            <h1>K.I.T's College of Engineering (Autonomous), Kolhapur</h1>
            <p>Academic Timetable | Dept. of CSE - AIML</p>
        </div>

        <div class="meta-bar">
            <span>STUDENT: <%= (student != null) ? student.toUpperCase() : "GUEST" %></span>
        ...
        <div class="grid-area">
            <%
                ...
                try (Connection con = DBConnection.getConnection()) {
                    String sql = "SELECT t.day_of_week, t.slot_id, t.room_no, t.is_lab, s.subject_name, f.faculty_name " +
                                 "FROM timetable t " +
                    ...
                    while(rs.next()) {
                        String key = rs.getString("day_of_week") + "_" + rs.getInt("slot_id");
                        scheduleMap.put(key, new ScheduleEntry(rs.getString("subject_name"), rs.getString("faculty_name"), rs.getString("room_no"), rs.getBoolean("is_lab")));
                    }

                    Statement st = con.createStatement();
                    ResultSet rsSlots = st.executeQuery("SELECT * FROM time_slot ORDER BY slot_order");
            %>
            <table class="table">
                <thead>
                    <tr>
                ...
                <%
                    ...
                    while(rsSlots.next()) {
                        int sid = rsSlots.getInt("slot_id");
                        String type = rsSlots.getString("slot_type");
                        String time = rsSlots.getString("start_time").substring(0, 5) + "-" + rsSlots.getString("end_time").substring(0, 5);
                %>
                <tr class="<%= !"LECTURE".equals(type) ? "r-break" : "r-lec" %>">
                    ...
                    %>
                            <td rowspan="2" class="lab-box">
                                ...
                                <span class="t-sub"><%= entry.name != null ? entry.name : "N/A" %></span>
